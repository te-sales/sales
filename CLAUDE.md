...
✍️ **ลายเซ็นหัวหน้า (phase 3.18 · 28 ก.ค. 2569):** ตาราง `signatures` (1 บัญชี 1 ลายเซ็น · data URL png · RLS อ่านทุกคน · **เขียน/ลบ = admin หรือ เจ้าของลายเซ็นเอง** `is_admin() or profile_id=auth.uid()` · 31 ก.ค.) · **admin จัดการให้ทุกคนในหน้าตั้งค่าระบบ** (การ์ด "ลายเซ็นหัวหน้า" · เฉพาะ role admin/manager · ลบกด 2 ครั้ง) · **หัวหน้าจัดการของตัวเองในหน้าโปรไฟล์ของฉัน** (`profile.js` ช่อง "ลายเซ็นของฉัน" · โชว์เฉพาะ admin/manager) · ตอนพิมพ์ ถ้าผู้เซ็นมีลายเซ็น → วางรูปในคอลัมน์ **NEXT DOING** แถว `.pf-signoff-row` (ใหญ่ ~2 บรรทัด · เอียง `--sign-rot:-15deg` · `.pf-sign-img` ใน print.css · map ผ่าน `signoff.signed_by`) · โหลดครั้งเดียวต่อการพิมพ์ (`loadSigMap` ใน formprint) · `fileToDataUrl` มี option `mime` (png คงพื้นหลังโปร่งใส) · ⚠️ ต้องรัน `db/phase3-18.sql` (ไม่รัน adapter คืน [] ไม่พัง)
