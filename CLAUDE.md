...
---
## สถานะโครงการ
**ล่าสุด: Book 3 สี เพิ่มเรียงตาม "สี"/"ผู้ดูแล" (v0.67.1) — จบ roadmap ครบทุก step แล้ว · งานล่าสุดเป็นของนอก roadmap ที่เจ้าของขอ · เหลือแค่ 1.7 ทดสอบเครื่องจริง (iPhone/S24/iPad)**
↕️ **Book 3 สี เรียงฝั่งเบราว์เซอร์ทั้งหมด** (`sortRows()` ใน book3.js · คลิกหัวตาราง ชื่อ/หน่วยงาน/สี/ผู้ดูแล) — **สี** เรียงตามระดับ `COLOR_RANK` (สนิท→เพิ่งเริ่ม) ไม่ใช่ a-z · **ผู้ดูแล** เรียงตามชื่อ `ownerName(sale_id)` ไม่ใช่ UUID · เสมอ→ใหม่ก่อน (updated_at) · ห้ามส่ง sort พวกนี้ไป DB (ไม่มีคอลัมน์ 'sale' · 'color' เรียง a-z ผิด)
📝 **หน้า "รอบันทึก" (P13 · `docs/js/modules/prep.js` · แท็บ ◔ ระหว่าง Book3/บันทึกด่วน):** คิว "จดชื่อไว้ก่อน → ทยอยเปิดมาบันทึกจริง"
   **ใช้ staging `intake_items` (3.5) เดิม 100% — ไม่มีตารางใหม่ ไม่ต้อง migration** (แค่ต้องรัน `db/phase3-5.sql` เหมือนเดิม)
   · จดชื่อเร็ว = `saveIntake({source:'manual',status:'draft'})` (Book3/Pending · กรอกแค่ชื่อ ไม่พึ่ง AI) · เปิดบันทึก = `openAIImport(target_type,{onSaved})` เด้งแท็บ staging เดิม (บันทึก/เช็คซ้ำ/ปิด draft→merged **ไม่ก๊อป**) · ทิ้ง = `rejectIntake` (2 คลิก)
   · เห็นเฉพาะ draft+approved · ค่าเริ่มต้น = ของฉัน (`ownerField:'created_by'`) สลับทีม/คนด้วย teamscope/personscope เดิม · บอกจำนวน "นอกทีมที่เลือก" ไม่ซ่อนเงียบ
...
🔎 **ตัวกรองขอบเขต Pending/Book3 = คอมโพเนนต์ร่วม 2 ตัว ใช้ต่อกัน (ทีม → คน) ห้ามก๊อป**
   ...
   ทั้งคู่คืน `{selected(), filter(rows)}` · teamscope ซ่อนเมื่อ ≤1 ทีม · **personscope โผล่เสมอถ้ามี ≥1 คน** (การันตีมี "ตัวเรา" จาก session เสมอ เผื่อ listProfiles คืนไม่ครบ — แก้ 27 ก.ค. 2569 "ดรอปดาวน์ไม่มา")
   · เลือกรายคน **ห้ามใช้ `sale_name` (free text จับคู่พลาด)** — ใช้ FK บัญชี เหมือนดรอปดาวน์เป้ารายคนบนหน้าภาพรวม
   · **ค่าเริ่มต้น = ตัวเอง** (เจ้าของสั่ง 27 ก.ค. 2569 · `defaultSelf` ใน personscope): เปิดหน้ามาเลือก meId (ถ้าอยู่ในลิสต์ที่เห็น) → แสดงงาน/ลูกค้าของ user ที่ล็อกอิน
   · **รายชื่อในดรอปดาวน์กรองตามทีมที่เลือก** (`setTeam()` · เจ้าของสั่ง 27 ก.ค. 2569): เลือก "รวมทุกทีม" → เห็นทุกคน · เลือก GOV.1 → เฉพาะสมาชิก GOV.1 (subtree ให้สิทธิ์ทีมแม่เห็นทีมลูก) · teamscope onChange ต้องเรียก `pscope.setTeam(id)` แล้ว sync `view.person` · dashboard ใช้ `renderPersonOptions()` เรียกตอนสลับทีม (เหมือนกันทั้ง 3 หน้า)
...
🧭 **สิทธิ์ "ดูทีมไหนได้" ต้องถามผ่าน `can_access_team()` เสมอ ห้ามเขียน `team_id = my_team_id()` เอง**
   ...
   (phase3-2 แก้ครั้งแรก · **27 ก.ค. 2569 แก้ที่ต้นทาง `policies.sql` ให้ profiles_select ใช้ can_access_team เลย** → รัน policies.sql ซ้ำไม่ revert อีก)
   · เขียนมือทีไร ตารางนั้นจะหลุดจากกติกากลางทันที
   🔴 **อาการที่เจอ:** ให้สิทธิ์ team_access ครบแล้ว "งาน/ลูกค้า" เห็นข้ามทีมได้ แต่ **ดรอปดาวน์ "ดูของ (รายคน)" ขึ้นแค่ตัวเอง** = profiles_select ยังเป็นเวอร์ชัน team_id=my_team_id() (ถูก policies.sql รันทับ) → รัน `db/fix-cross-team-access.sql` (รวม profiles_select แล้ว)
...
📰 **ข่าวสารโอกาสงานประจำสัปดาห์ (news_reports · phase3-14):** เจ้าของสร้างรายงาน HTML จาก Claude Code ทุกสัปดาห์ → แถบแหล่งงาน **แท็บ "📰 ข่าวประจำสัปดาห์" (แท็บแรก · ค่าเริ่มต้น)** · admin กด "+ เพิ่มข่าว" → **เลือกไฟล์ .html (อ่านเป็น UTF-8 กันไทยเพี้ยน) หรือวางโค้ด** (ดึงชื่อจาก `<title>`) · ใหม่สุด = การ์ดไฮไลต์ "🆕 ใหม่สัปดาห์นี้" · เก่า = "ข่าวย้อนหลัง" · เปิดอ่านเต็มจอผ่าน `<iframe srcdoc>` sandbox (ไม่มี allow-same-origin = แยก origin) · admin ลบได้ (กด 2 ครั้งยืนยัน)
...
☁️ **สำรองขึ้น Google Drive อัตโนมัติ (task 5 · Edge Function `daily-backup` + pg_cron):** เลือกแนวทาง "อัตโนมัติจริงฝั่งเซิร์ฟเวอร์" (26 ก.ค. 2569) — เว็บ static สั่ง Drive เองไม่ได้ · ดึงทุกตารางด้วย service_role → อัปขึ้น Drive ด้วย **Google service account** → log `backup_log` · หน้า Admin โชว์สถานะ + ปุ่ม "สำรองเดี๋ยวนี้"
   ...
   ✅ **ยืนยันใช้งานจริงแล้ว (26 ก.ค. 2569):** อัปขึ้น Drive สำเร็จ (te-backup-*.json เจ้าของ theerasaku@gmail.com) + ตรวจ byte จริง = UTF-8 ไทยถูกต้อง · ใช้ **OAuth refresh token** (Gmail ส่วนตัว · service account เจอ storageQuotaExceeded) + ต้องรัน **`db/grant-service-role.sql`** (service_role ไม่มี grant → 403) · เหลือเจ้าของตั้ง pg_cron ('0 19 * * *' = ตี 2 ไทย) ให้อัตโนมัติ
   🔑 **พรีวิว .json ของ Google Drive โชว์ mojibake (Latin-1) ทั้งที่ไฟล์ UTF-8 ดี** — อย่าวินิจฉัยไฟล์เสียจากพรีวิว Drive · เช็ก byte จริง · 🔒 คีย์อยู่ใน Supabase secrets เท่านั้น ห้ามใส่ repo
📄 **ดูแบบฟอร์มบนจอ + พิมพ์รวม (28 ก.ค. 2569):** ปุ่มในฟอร์ม = "📄 ดูแบบฟอร์ม / พิมพ์" (เปิดพรีวิว `doPrint` เดิม = มุมมองฟอร์มต้นฉบับ + พิมพ์ในตัว) · ปุ่มในลิสต์ = "🖨 พิมพ์ทั้งหมด" รวมทุกรายการที่กรองอยู่เป็น PDF เดียว (`printPendingBatch`/`printCustomerBatch` · ต่อ `.pf-page` คั่นหน้าอัตโนมัติ) — **ห้ามก๊อป formprint ไปเขียนซ้ำ** ใช้ `pendingRecord`/`customerRecord` ร่วม
👤 **โปรไฟล์ของฉัน (28 ก.ค. 2569):** `js/ui/profile.js` `openProfile(user, onNameSaved)` — ปุ่ม 👤 บนแถบหัว(`#profileBtnTop`)/แถบข้าง(`#profileBtn`) · แก้ **ชื่อที่แสดง** (`saveProfile` own/admin) + **รหัสผ่าน** (`updatePassword` ใช้ session token) เองในแอป · **ไม่ต้อง migration** (policy/ trigger เดิมรองรับ) · แก้ชื่อแล้ว `paintUser` อัปเดตแถบหัวทันที · อีเมล(login)แก้ที่นี่ไม่ได้
✍️ **ช่องข้อความมีรูปแบบ (rich text · 27 ก.ค. · ปรับ 28 ก.ค. 2569):** `js/ui/richtext.js` = `richFieldHtml`/`sanitizeHtml`/`richToText`/`richBlank` · **ใช้เฉพาะ RESPONSE / NEXT DOING ในบันทึกติดตามเท่านั้น** (เจ้าของสั่ง 28 ก.ค. — ถอดออกจากช่อง `area` ทั้งหมดแล้ว ช่อง area กลับเป็น textarea ธรรมดา · ค่าเก่าที่เป็น HTML ตัดเป็นข้อความล้วนด้วย `richToText` ตอนโชว์)
   🔴 **wrapper ต้องเป็น `<div>` ห้ามเป็น `<label>`** — contenteditable ใน `<label>` โดนเบราว์เซอร์เด้ง focus ไปปุ่มแรกในเลเบล = "พิมพ์ในบล็อกไม่ได้" (พิสูจน์ด้วย CDP คลิก+คีย์จริง 28 ก.ค.)
   ⚡ event ผูกแบบ **delegation ที่ document ครั้งเดียว** (`installGlobalRich` · `bindRichFields` เหลือเป็น shim) → ช่องที่สร้างทีหลัง (add-log/edit-log) ใช้ได้เลย · `richBlank` = ว่างจริงไหม (contenteditable ว่างเหลือ `<br>` ต้องไม่นับ) ใช้เช็คก่อนบันทึก log ทุกจุด
   · RESPONSE/NEXT DOING มี 5 จุดที่เป็น rich: loglist `logFormHtml`+`bindLogEditing` (book3/quicklog) · pending qLog panel (name=response/next_doing → FormData) · pending full-modal (`#lgRes/#lgNext` **id-only ไม่มี name** กันไปปน payload งาน) · แสดงผ่าน `sanitizeHtml` · loghover/พิมพ์ตัดเป็นข้อความล้วน
✍️ **(เดิม 27 ก.ค.) rich text เคยใช้กับช่อง type `area`** — ยกเลิกแล้ว (ดูบรรทัดบน)
   🔒 **เก็บเป็น HTML แต่ sanitize ทุกครั้ง** (allowlist: b/strong/i/em/u/mark/span + style เฉพาะ color/background-color ในพาเลตคงที่ · ตัด script/on*/href/img/font นอกพาเลต) — repo public + ข้อมูลหลัง RLS ห้ามให้ XSS หลุด
   · ตาราง/CSV ใช้ `richToText()` (ตัดแท็ก) · พิมพ์ใช้ `sanitizeHtml()` ผ่าน `VH()` ใน formprint · **project_detail/education ตอนพิมพ์ตัดเหลือข้อความล้วนก่อนแบ่งบรรทัด** (ไม่งั้น <div>/<br> พังการ split)
   · execCommand ใช้ `styleWithCSS` ให้ foreColor/hiliteColor คืน `<span style>` (ไม่ใช่ `<font>`) · sanitize รับ `<font color>` เป็น fallback
🏷️ **DOCC (ประเภทลูกค้า · Book 3 สี · 27 ก.ค. 2569):** Designer/Owner/Contractor/Consult — เก็บ "คำเต็ม" (`db/phase3-17.sql`) เพราะตัว C ซ้ำ 2 แบบ · หน้าจอโชว์ตัวย่อ D/O/C/C · ดรอปดาวน์กรอง `#bDocc` (กรองฝั่งเบราว์เซอร์ใน applyScopes)
🗑 **ลบรูปนามบัตร/รูปลูกค้า = กดยืนยัน 2 ครั้ง + สิทธิ์เฉพาะ admin/เจ้าของ** (27 ก.ค. 2569) — `bindCardField`/`bindPhotoField` รับ `canDelete` · book3 คำนวณ `!id || admin || !sale_id || sale_id===me`
📊 **ป้ายจำนวน (สี 🟢🟡🔴 · Archive) นับตามขอบเขตทีม/คนที่เลือกเท่านั้น** (27 ก.ค. 2569) — เดิมนับทั้งหมดที่ RLS เห็น → เลขป้ายไม่ตรงกับที่กดเข้าไปดู · pending/book3 มีโน้ต "มีในคลังนอกทีม N" (ห้ามซ่อนเงียบ)
🖨️ **พิมพ์: ข้อมูลที่ user กรอก = สีน้ำเงิน `#1b53c0`** (print.css `.pf-v`/`.pf-tbl td` · ป้าย/หัวตารางคงดำ) · **หน้า 2+ มีหัว `.pf-hdr-name` ชื่อ-สกุล/หน่วยงาน** · **แถวหัวหน้าเซ็นรับทราบ = สีเขียว `#1f7a44`** (`.pf-signoff-row td` · เจ้าของสั่ง 28 ก.ค. 2569 · คลาสร่วมครอบทั้ง Book 3 สี + Pending)
✍️ **ลายเซ็นหัวหน้า (phase 3.18 · 28 ก.ค. 2569):** ตาราง `signatures` (1 บัญชี 1 ลายเซ็น · data URL png · RLS อ่านทุกคน · **เขียน/ลบ = admin หรือ เจ้าของลายเซ็นเอง** `is_admin() or profile_id=auth.uid()` · 31 ก.ค.) · **admin จัดการให้ทุกคนในหน้าตั้งค่าระบบ** (การ์ด "ลายเซ็นหัวหน้า" · เฉพาะ role admin/manager · ลบกด 2 ครั้ง) · **หัวหน้าจัดการของตัวเองในหน้าโปรไฟล์ของฉัน** (`profile.js` ช่อง "ลายเซ็นของฉัน" · โชว์เฉพาะ admin/manager)
   · **ตัดพื้นหลังขาวออก** ตอนอัป (`fileToDataUrl` opt `removeWhiteBg` · alpha ตาม luminance · เหลือแต่ลายเส้น กันกล่องขาวหมุนทับเส้นตาราง) · **ปรับขนาดได้จากหน้าตั้งค่า** (สไลเดอร์ → `app_settings.signature.size_pct` → formprint เซ็ต `--sign-scale` บน `#printRoot` → print.css `calc(52pt * var(--sign-scale))` · 100%=ฐาน) · ตอนพิมพ์ ถ้าผู้เซ็นมีลายเซ็น → วางรูปในคอลัมน์ **NEXT DOING** แถว `.pf-signoff-row` (ใหญ่ ~2 บรรทัด · เอียง `--sign-rot:-15deg` · `.pf-sign-img` ใน print.css · map ผ่าน `signoff.signed_by`) · โหลดครั้งเดียวต่อการพิมพ์ (`loadSigMap` ใน formprint) · `fileToDataUrl` มี option `mime` (png คงพื้นหลังโปร่งใส) · ⚠️ ต้องรัน `db/phase3-18.sql` (ไม่รัน adapter คืน [] ไม่พัง)
🔌 **`docs/js/data/adapter.js` = allowlist ชัดเจน ไม่ใช่ proxy** — เพิ่ม method ใน supabase/local-adapter แล้ว **ต้องลงทะเบียนใน facade `adapter` ด้วย** ไม่งั้น UI เรียก `adapter.xxx()` ได้ undefined (เงียบ ๆ ไม่มี error ชัด · เสียเทสต์ 1 รอบตอนทำ 3.18)
⚡ **บันทึกด่วน (quicklog): sale เห็น/เลือกเฉพาะของตัวเอง** (`ownerField` · role==='sale' filter) · หัวหน้า/admin เห็นหมด (27 ก.ค. 2569)
📐 **`.modal-body{flex:1;min-height:0}` = ปุ่มบันทึกท้ายฟอร์มไม่ถูก overflow ตัดหาย** บนฟอร์มยาว (เจ้าของขอ "บันทึกตรงไหนก็ได้" 27 ก.ค. 2569) — ห้ามถอด flex ออก
⚠️ ต้องรัน `db/phase3-17.sql` ใน Supabase — คอลัมน์ `customers.docc` · ไม่รันก็ยังบันทึกลูกค้าได้ (adapter ตัด docc ออกอัตโนมัติ) แค่ยังเก็บ/กรอง DOCC ไม่ได้
🪪 **นามบัตร Book 3 สี (2 รูป หน้า/หลัง · 27 ก.ค. 2569):** `js/ui/cardfield.js` (เก็บ `customers.card_front_url`/`card_back_url` เป็น data URL ย่อ 1280px) + `js/ui/lightbox.js` (ดูเต็มจอ+ซูม/แพน/ปินช์ · พื้นหลังมืดคงที่ hardcode ได้เหมือน .modal) · desktop/iPad = ขวาของรูปบุคคล · มือถือ = ใต้รูป (`.photo-card-row` + media 640px)
   ⚠️ ต้องรัน `db/phase3-16.sql` ใน Supabase — ไม่รันก็ยังใช้ได้ (saveCustomer ตัด card_*/age ออกอัตโนมัติถ้า column ยังไม่มี) แค่ยังเก็บนามบัตรไม่ได้
   📋 **log สำรอง Google Drive ย้ายเป็นหน้าใน (modal)** ในหน้า Admin แล้ว (ปุ่ม "ดูประวัติการสำรอง") — ไม่ให้รายการยาวรกหน้า setting
