...
## 2026-08-22 · ยังไม่ commit · Book 3 สี: เพิ่มเรียงตาม "สี" และ "ผู้ดูแล"
**ประเภท:** ฟีเจอร์ | **เวอร์ชัน:** 0.67.1
- หัวตาราง **สี** + **ผู้ดูแล** กดเรียงได้ (เดิมกดได้แค่ ชื่อ/หน่วยงาน) · toggle asc/desc + ลูกศร ▲▼
- **สี เรียงตามระดับ** (สนิท🟢→มีโอกาส🟡→เพิ่งเริ่ม🔴) ด้วย `COLOR_RANK` ไม่ใช่ a-z (green<red<yellow ผิดความหมาย)
- **ผู้ดูแล เรียงตามชื่อจริง** (resolve จากโปรไฟล์ `ownerName(sale_id)` + fallback sale_name) ไม่ใช่ UUID · localeCompare 'th'
- ย้ายการเรียงทั้งหมดมาทำ**ฝั่งเบราว์เซอร์** ใน `sortRows()` (applyScopes) — คลิกหัวตาราง = เรียงทันที ไม่โหลด DB ใหม่ (name/org เดิมก็ใช้ทางนี้ · สม่ำเสมอ) · เสมอกัน→ใหม่ก่อน (updated_at)
**ไฟล์:** docs/js/modules/book3.js · docs/js/config.js + docs/sw.js (v0.67.1)
**ทดสอบ:** CDP โหมด local 10/10 ผ่าน · ไม่มี JS error (สี asc/desc · ผู้ดูแลไทย asc/desc · ชื่อยังเรียงได้)
